"use strict";
// npm run test challenges-08.test.js
/* ------------------------------------------------------------------------------------------------
CHALLENGE 1 - Review
...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  return arr.filter((str) => str.match(/[aeiou]/gi));
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
 return arr.filter((ele)=> !forbiddenValues.find((num)=> num ==ele))
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  const baseStat= arr.filter((stat)=> stat.baseStat > minBaseStat && stat)
  return baseStat.map((ele) => ele.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  ...
  return arr.filter((ele)=> typeof ele == 'number').map((ele)=> ele % 2 ==0 ? 'even' : 'odd')
};
