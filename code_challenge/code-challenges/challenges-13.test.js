"use strict";
// npm run test challenges-13.test.js
/* ------------------------------------------------------------------------------------------------
CHALLENGE 1 - Review
...
const longestString = (arr) => {
  // Solution code here...
  // return arr.map((longest , idx)=> Math.max(...longest)[idx] )
  let index = 0; 
  if (arr.length > 0){
    arr.map((str , idx)=> {
      if (arr[index].length < str.length) index = idx
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map((phoneNum) => phoneNum.replace(/\s|\(|\)|\-/g, ""));
};
...
const sortByDay = (arr) => {
  ...
  daysOfWeek.map((day , idx)=> {
    res[idx] = [];
    arr.forEach(event => {
      if(event.includes(day)) res[idx].push(event)
