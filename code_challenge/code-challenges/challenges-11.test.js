"use strict";
// npm run test challenges-11.test.js
/* ------------------------------------------------------------------------------------------------
CHALLENGE 1 - Review
...
const validateEmail = (email) => {
  // Solution code here...
if (! /^(?!(\.*\.){2})(.*\.){3}.*$/.test(email)) return /[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+(?:[A-Z]{2}|com|org|net)\b/.test(email)
};
...
const validatePhoneNumber = (phoneNumber) => {
  ...
  if (phoneNumber.match(/\d/g) && phoneNumber.match(/\d/g).length == 10)
    return (
      /^([0-9]{3,6})[- ]?([0-9]{3})[- ]?([0-9]{4})$/g.test(phoneNumber) ||
      /^\(([0-9]{3,6})\)[- ]?([0-9]{3})[- ]?([0-9]{4})$/g.test(phoneNumber)
    );
};
...
const findTagNames = (elements) => {
  // Solution code here...
  const newArr = elements.map((tag) => tag.match(/<\/.+?>/g)); 
  return [].concat(...newArr).map((tag)=> tag.slice(1, tag.length-1))
};
