"use strict";
// npm run test challenges-10.test.js
/* ------------------------------------------------------------------------------------------------
CHALLENGE 1 - Review
...
const count = (target, input) => {
  ...
  return input.reduce((acc , cur)=> {
    const found = cur.filter((ele)=> ele === target);
    acc+= found.length;
    return acc;
  },0)
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  //   ele.filter((num)=> typeof num == 'number' && num % 5 == 0).map((item)=> Math.pow(2 , item))
  // })


  // const divisibles = input.map((item)=> item.filter((num)=> typeof num == "number" && num % 5 == 0));
  // return divisibles.map((ele)=> Math.pow(2 , ele))
...
let findShortest = (data) => {
 ...
 const shortestOne = data.reduce((acc , cur)=> Number(acc.height)< Number(cur.height)? acc : cur);
 return  shortestOne.name;
