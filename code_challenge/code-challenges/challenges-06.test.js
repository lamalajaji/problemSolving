'use strict';
// npm run test challenges-06.test.js
/* ------------------------------------------------------------------------------------------------
...
const sortByChildren = (charArray) => {
	// Solution code here...
	return charArray.sort((a , b)=> a.children.length - b.children .length|| a.house - b.house);
...
const totalCharacters = (arr) => {
	...
	arr.map((char)=> {
		if (char.name) charactersCounter +=1;
		if(char.spouse) charactersCounter +=1; 
		if (char.children) charactersCounter += char.children.length
	})
...
const houseSize = (arr) => {
	...
	// 	if (member.spouse) members += 1;
	// 	if(member.children) members += member.children.length;

	// 	sizes.push({house : member.house ,members})
...
const houseSurvivors = (arr) => {
  ...
  // Solution code here...
  // deceasedSpouses.find((member)=> {
  // 	if (member === house.spouse){
  // 		if(house.name) members += 1;
  // 		if(house.children) members += house.children.length;
  // 		survivors.push({house: house.name , members})
  // 	} else {
 ...
 const survivors = [];
  arr.forEach((house) => {
    let members = 0;
    if (deceasedSpouses.find((name) => name != house.spouse)) {
      if (house.name) members += 1;
    }
